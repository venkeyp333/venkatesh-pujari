...
export const Skills = () => {
  ...
  const [visibleWords, setVisibleWords] = useState(Array(words.length).fill(false));
  ...
  return (
    <section className={`${darkMode ? 'bg-gray-900' : 'bg-gray-100 '} py-20 relative`} id="skills" ref={ref}>
      <div className="container mx-auto px-6">
        <div className="text-center mb-12">
          <h2 className={`${darkMode ? 'text-white' : 'text-gray-900'} text-4xl font-bold`}>Skills</h2>
        </div>
        ...
        <div className="flex flex-wrap justify-center mb-10">
          {words.map((word, index) => (
            <motion.span 
              ...
              animate={visibleWords[index] ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }} // Animate based on visibility
              transition={{ duration: 0.5, delay: index * 0.1 }} 
              className={`${darkMode ? 'text-gray-300' : 'text-black'} text-xl font-medium mr-2 mb-2`}
            >
              {word}
            </motion.span>
          ))}
        ...
        <div className="mb-10">
          <h3 className={`${darkMode ? 'text-white' : 'text-gray-900'} text-2xl font-semibold mb-4`}>Programming Languages</h3>
          <Carousel 
          ...
          >
            {skillsDetailed.programmingLanguages.map((skill, index) => (
              <div className="item flex flex-col items-center" key={index}>
                <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} w-24 h-24 flex justify-center items-center mb-3 rounded-full shadow-lg`}>
                  <img src={skill.icon} alt={skill.name} className="w-2/3 h-2/3" />
                </div>
                <h5 className={`${darkMode ? 'text-gray-300' : 'text-gray-900'} text-lg font-semibold`}>{skill.name}</h5>
              </div>
            ))}
          </Carousel>
        </div>
        ...
        {/* Frontend Section */}
        <div className="mb-10">
          <h3 className={`${darkMode ? 'text-white' : 'text-gray-900'} text-2xl font-semibold mb-4`}>Frontend</h3>
          <Carousel 
            responsive={responsive} 
            infinite={true} 
            className="owl-carousel owl-theme skill-slider" 
        ...
        {/* Database Section */}
        <div className="mb-10">
          <h3 className={`${darkMode ? 'text-white' : 'text-gray-900'} text-2xl font-semibold mb-4`}>Database</h3>
          <Carousel 
            responsive={responsive} 
            infinite={true} 
            className="owl-carousel owl-theme skill-slider" 
